...
//    			if(cname.equalsIgnoreCase(tc.getChapterName().trim())){
    			boolean eq = false;
    			if(GlobalConfig.collect.getBoolean("shortname", false)) {
    				eq = ParseHelper.compareChapterName(tc.getChapterName(), cname);
    			} else {
    				eq = cname.equalsIgnoreCase(tc.getChapterName().trim());
    			}
    			if(eq) {
    				needCollect = false;
    				break;
	...
	private void collectChapter(String novelNo,String cno, String novelPubKeyURL, 
		...
		Integer chapterNo = 0;
		//章节号不存在说明入口是正常采集normalCollect， 此时判断数据库中如果存在对应章节则说明已经采集过， 直接返回
		...
		if(StringUtils.isNotBlank(chapterURL)) {
			...
			if(StringUtils.isBlank(chapterContent)) {
				logger.error("采集到空章节， 规则：{}, 小说名：{}, 章节名：{}目标地址：{}, ",
						cpm.getRuleMap().get(Rule.RegexNamePattern.GET_SITE_NAME).getPattern(), 
						novel.getNovelName(), chapter.getChapterName(), chapterURL);
			} else {
