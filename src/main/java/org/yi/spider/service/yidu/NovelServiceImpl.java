...
public class NovelServiceImpl extends BaseService implements INovelService {
    ...
    private final class NovelEntityResultSetHandler implements ResultSetHandler<NovelEntity> {
        @Override
    ...
    public Number saveNovel(NovelEntity novel) throws SQLException {
        ...
        Object[] params = new Object[] { novel.getNovelName(), novel.getPinyin(), novel.getInitial(),
                StringUtils.trimToEmpty(novel.getKeywords()), 0, novel.getAuthor(), novel.getTopCategory(),
                novel.getSubCategory(), novel.getIntro(), novel.getFullFlag(),
                new Timestamp(System.currentTimeMillis()), 0, 0, 0, 0, 0, 0, 0, 0 };

        return queryRunner.save(conn, sql, params);// 因为stmt.getGeneratedKeys();返回的主键id是Long，直接转化成Integer或跑错
    }
    ...
    public Number getMaxPinyin(String pinyin) throws SQLException {
        ...
        String sql = "SELECT count(*) FROM t_article WHERE pinyin regexp '^" + pinyin + "\\d*' ";
